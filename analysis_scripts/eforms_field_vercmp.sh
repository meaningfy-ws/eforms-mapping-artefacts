...
eforms_field_diff() {
    ...
    old=$(git co -q $earliest && cat fields/fields.json | jq -r --arg ID "$bt" '.fields[] | select(.id == $ID)')
    new=$(git co -q 1.9.1 && cat fields/fields.json | jq -r --arg ID "$bt" '.fields[] | select(.id == $ID)')
    ...
    if [[ -n $3 ]]; then
        ...
        mid=$(git co -q $latest && cat fields/fields.json | jq -r --arg ID "$bt" '.fields[] | select(.id == $ID)')
